...
class Pay
{
     ...
     * @param UnifiedOrderData $inputObj
     * @param int $timeOut
     * @throws Exception
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        $inputObj->IsNotify_urlSet() || $inputObj->SetNotify_url(Config::$NOTIFY_URL);//异步通知url
    ...
    {
        //获取通知的数据
        // HTTP_RAW_POST_DATA is deprecated since PHP5.6.0
        // $xml = $GLOBALS['HTTP_RAW_POST_DATA'];
        $xml = file_get_contents('php://input');
        //如果返回成功则验证签名
        try {
            $result = Result::Init($xml);
        } catch (Exception $e) {
            $msg = $e->getMessage();
            return false;
        }
    ...
    public static function transfer($transfer)
    {
        ...
        //检测必填参数
        if ($transfer->valueExists('partner_trade_no') == null) {   //
            throw new Exception("退款申请接口中，缺少必填参数partner_trade_no!");
        } else if ($transfer->valueExists('openid') == null) {
            throw new Exception("退款申请接口中，缺少必填参数openid!");
        } else if ($transfer->valueExists('check_name') == null) {  // NO_CHECK：不校验真实姓名 FORCE_CHECK：强校验真实姓名（未实名认证的用户会校验失败，无法转账）OPTION_CHECK：针对已实名认证的用户才校验真实姓名（未实名认证用户不校验，可以转账成功）
            throw new Exception("退款申请接口中，缺少必填参数check_name!");
        } else if (in_array($transfer->getCheck_name(), ['FORCE_CHECK', 'OPTION_CHECK']) && !$transfer->valueExists('re_user_name')) {
            throw new Exception("退款申请接口中，缺少必填参数re_user_name！");
        } else if ($transfer->valueExists('amount') == null) {
            throw new Exception("退款申请接口中，缺少必填参数amount!");
        } else if ($transfer->valueExists('desc') == null) {
            throw new Exception("退款申请接口中，缺少必填参数desc!");
        }

        $transfer->setMch_appid(Config::$APP_ID)
            ->setMchid(Config::$MCH_ID)
            ->setSpbill_create_ip('127.0.0.1')
            ->setNonce_str(static::getNonceStr())
            ->SetSign();

        $xml = $transfer->ToXml();
        $response = static::postXmlCurl($xml, $url, true);
        $result = new Result();
        return $result->xmlToArray($response);
    }
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if ($data) {
            ...
            $errno = curl_errno($ch);
            curl_close($ch);
            throw new Exception("curl出错，错误码:$error($errno)");
        }
    }
